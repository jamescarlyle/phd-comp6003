...
class CartPole:
    ...
    def get_scaled_state(self):
        """Get full state for swing-up, scaled into (approximately) [0, 1]."""
        return [
            # Cart Position: scaled [0, 1]
            0.5 * (self.x + self.position_limit) / self.position_limit,
            
            # Cart Velocity: scaled [0, 1] (Assuming -1.5 to 1.5 range)
            (self.dx + 1.5) / 3.0,
            
            # Pole Angle Sine: scaled [0, 1]
            (sin(self.theta) + 1.0) / 2.0,
            
            # Pole Angle Cosine: scaled [0, 1]
            (cos(self.theta) + 1.0) / 2.0,
            
            # Pole Angular Velocity: scaled [0, 1] (Assuming -5.0 to 5.0 range)
            (self.dtheta + 5.0) / 10.0
        ]
